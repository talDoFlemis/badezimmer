...
export function DeviceCard({ device }: { device: ConnectedDevice }) {
  ...
  // const convertArrayToMap = (arrayOfPairs: [][]string) => {
  //   // O método 'reduce' é ideal para transformar um array em um objeto
  //   return arrayOfPairs.reduce((acc, currentPair) => {
  //     // currentPair é o array interno, ex: ["category", "FART_DETECTOR"]
  //     const key = currentPair[0];
  //     const value = currentPair[1];
  //
  //     // Adiciona o par chave-valor ao acumulador (acc)
  //     acc[key] = value;
  //
  //     // Retorna o objeto atualizado para a próxima iteração
  //     return acc;
  //   }, {}); // O objeto vazio ({}) é o valor inicial do acumulador
  // }
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            ...
            <CardDescription className="text-slate-400">
              {deviceCategoryNames[device.category]}
            </CardDescription>
          </div>
          <Badge
            className={`${deviceStatusColors[device.status]} text-white border-0`}
          >
